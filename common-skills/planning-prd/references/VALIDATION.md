...
Before a PRD can proceed to downstream planning phases, it must pass all 8 validation dimensions at "high" confidence. This is a blocking gate — not advisory.

The PRD agent runs this cycle after each PRD draft or significant update, presents a scorecard, asks targeted probing questions for weak dimensions, and iterates until all dimensions reach "high".
...
**Common failure patterns**:
- "We'll use React Native for V2" without addressing V1-to-V2 migration path feasibility
...
**Research dispatch**: When the agent cannot confidently assess technical feasibility, dispatch an `sdlc-project-research` agent before scoring this dimension.
