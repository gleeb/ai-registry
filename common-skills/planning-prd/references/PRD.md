...
## Purpose
Use this format when drafting or refining a Product Requirements Document.
...
- REQUIRE explicit decomposition intent into User Stories.
- DENY vague language that cannot be validated from external evidence or observable criteria.
- DENY proceeding to user story decomposition before passing all 8 validation dimensions at "high" (see [`VALIDATION.md`](VALIDATION.md)).
- ALLOW provisional local PRD draft only when clearly marked `PROVISIONAL - NOT VALIDATED`.
...
Group user stories by feature area. Each group becomes a user story document in `plan/user-stories/`.
...
- REQUIRE user story groups in section 7 form a coherent set of user story documents.
- REQUIRE all 8 validation dimensions pass at "high" before proceeding to downstream planning.
