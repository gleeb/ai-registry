...
This file is loaded by OpenCode at the start of every session.
...
- Use `/sdlc <project>` or switch to the `sdlc-coordinator` agent to enter the SDLC workflow.
- Use `/sdlc-continue` to resume from the last checkpoint.
